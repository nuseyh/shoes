...
//      System.out.println("cart_no:"+ no);
      ...
      boolean result = cart_count(cart.getProduct_no(), cart.getUser_id(), cart.getProduct_size());
...
//         System.out.println(cart);
         ...
         Object[] obj = {no, cart.getUser_id(), cart.getProduct_no(), cart.getProduct_name(), cart.getProduct_price(), cart.getProduct_size(), cart.getProduct_color(), cart.getCount(), cart.getProduct_price()*cart.getCount()};
...
//      jdbcTemplate.update(sql, obj);
   ...
   //장바구니 내의 같은 상품 확인
   @Override
   public boolean cart_count(int product_no, String user_id, int product_size) {
      String sql = "select * from cart where product_no=? and user_id=? and product_size=?";
      Object[] obj = {product_no, user_id, product_size};
