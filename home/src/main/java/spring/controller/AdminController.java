...
public class AdminController {
	...
	// public String mkdir() {
	// File f = new
	// File("C:\\Users\\IT202-24\\git\\shoes\\home\\src\\main\\webapp\\resource\\img\\i_name");
	// if(!f.exists()) {
	// f.mkdirs();
	//
	...
	// 판매목록
	@RequestMapping({ "/selllist", "/selllist/" })
	public String selllist(Model model) {
		model.addAttribute("selllist", adminDao.selllist());
		return "admin/selllist";
	}

	// 판매목록 검색
	@RequestMapping("/selllist/{key}")
	public String selllist(Model model, @PathVariable String key) {
		model.addAttribute("selllist", adminDao.search2(key));
		return "admin/selllist";
	}
